...
def get_filter(p2f, DT, WT):
    ...
    with open(p2f, mode='r') as f:
        ...
        for line in f:
            ...
            id_list[id_idx].append(case_id)

            line_count += 1
            if line_count % max_len == 0:
                id_idx += 1

    combined_filter = []
    for ll in id_list:
        op_1 = single_op("in", "cases.case_id", ll)
        ...
        combined_filter.append(base_filter)
...
if __name__ == '__main__':
    ...
    for ff in filt:
        params = {'filters':json.dumps(ff), 'format':FLAG.format,
        ...
        if FLAG.format == 'json':
            with open(FLAG.saveFile, 'a') as f:
                json.dump(response.json(), f, indent=2)
        else:
            with open(FLAG.saveFile, 'ab') as f:
